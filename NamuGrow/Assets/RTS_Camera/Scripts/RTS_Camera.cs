...
namespace RTS_Cam
{
    ...
    public class RTS_Camera : MonoBehaviour
    {
        ...
        {
            if (useKeyboardRotation == false) return; 
            targetPos = Util.ClickPos;
            if(useKeyboardRotation)
                transform.RotateAround(targetPos, Vector3.up, RotationDirection * Time.deltaTime * rotationSped);
            if (useMouseRotation && Input.GetKey(mouseRotationKey))
                m_Transform.RotateAround(targetPos,Vector3.up, -MouseAxis.x * Time.deltaTime * mouseRotationSpeed);
            //if(useKeyboardRotation)
